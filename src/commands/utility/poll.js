...
export default {
  ...
  async execute(message, args) {
    ...
    collector.on('collect', async (interaction) => {
      ...
      if (!poll) {
        return interaction.reply({ content: '**Notice:** This poll has concluded, Master.', ephemeral: true });
      }
      ...
      if (previousVote === optionIndex) {
        ...
        if (previousVote !== undefined) {
          await interaction.reply({
            content: `**Confirmed:** Vote changed from **${options[previousVote]}** to **${options[optionIndex]}**, Master.`,
            ephemeral: true
          });
        } else {
          await interaction.reply({
            content: `**Confirmed:** Vote registered for **${options[optionIndex]}**, Master.`,
            ephemeral: true
          });
        }
      }
  ...
  async finalizePoll(pollMsg, poll, options, optionEmojis) {
    ...
    let winnerText;
    if (totalVotes === 0) {
      winnerText = '◈ No votes were recorded.';
    } else if (winnerIndices.length > 1) {
      winnerText = `◈ **Result: Tie** between ${winnerIndices.map(i => options[i]).join(' & ')}`;
    } else {
