...
export default class SkipTo extends Command {
  ...
  async run(client, ctx, args) {
    ...
    logger.info(`[SkipTo Command] Target position: ${position}, queue size: ${queue.queue.length}`);
    ...
    logger.info(`[SkipTo Command] Target track: ${queue.queue[0]?.info?.title || 'unknown'}`);
