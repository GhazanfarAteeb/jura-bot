...
export default {
    ...
    async execute(message) {
        const embed = await infoEmbed(message.guild.id, 'Setting Up JURA BOT',
        ...
        try {
            ...
            if (!susRole) {
                susRole = await message.guild.roles.create({
                    name: '🚨 Sus/Radar',
                    color: '#ff9900',
                    reason: 'JURA BOT Setup - Suspicious member tracking'
                });
            ...
            if (!newAccountRole) {
                newAccountRole = await message.guild.roles.create({
                    name: '🥚 New Account',
                    color: '#ffff00',
                    reason: 'JURA BOT Setup - New account tracking'
                });
            ...
            if (!mutedRole) {
                mutedRole = await message.guild.roles.create({
                    ...
                    color: '#808080',
                    permissions: [],
                    reason: 'JURA BOT Setup - Mute functionality'
                });
            ...
            if (!modLogChannel) {
                modLogChannel = await message.guild.channels.create({
                    ...
                    reason: 'JURA BOT Setup - Moderation logging'
                });
            ...
            if (!alertLogChannel) {
                alertLogChannel = await message.guild.channels.create({
                    ...
                    type: ChannelType.GuildText,
                    parent: logsCategory.id,
                    reason: 'JURA BOT Setup - Security alerts'
                });
            ...
            if (!joinLogChannel) {
                joinLogChannel = await message.guild.channels.create({
                    ...
                    type: ChannelType.GuildText,
                    parent: logsCategory.id,
                    reason: 'JURA BOT Setup - Join/leave logging'
                });
            ...
            if (!botStatusChannel) {
                botStatusChannel = await message.guild.channels.create({
                    ...
                    type: ChannelType.GuildText,
                    parent: logsCategory.id,
                    reason: 'JURA BOT Setup - Bot status monitoring'
                });
            ...
            if (!birthdayChannel) {
                birthdayChannel = await message.guild.channels.create({
                    name: '🎂-birthdays',
                    type: ChannelType.GuildText,
                    reason: 'JURA BOT Setup - Birthday announcements'
                });
            ...
            if (!eventChannel) {
                eventChannel = await message.guild.channels.create({
                    name: '📅-events',
                    type: ChannelType.GuildText,
                    reason: 'JURA BOT Setup - Event announcements'
                });
            ...
            if (!levelUpChannel) {
                levelUpChannel = await message.guild.channels.create({
                    name: '🎉-level-ups',
                    type: ChannelType.GuildText,
                    reason: 'JURA BOT Setup - Level up announcements'
                });
            ...
            if (!welcomeChannel) {
                welcomeChannel = await message.guild.channels.create({
                    name: '👋-welcome',
                    type: ChannelType.GuildText,
                    reason: 'JURA BOT Setup - Welcome messages'
                });
            ...
            const successMsg = await successEmbed(message.guild.id, 'Setup Complete!',
                `${GLYPHS.SUCCESS} JURA BOT has been successfully set up!\n\n` +
            ...
            const welcomeEmbed = await infoEmbed(message.guild.id, 
                `${GLYPHS.SPARKLE} JURA BOT Activated`,
                `Thank you for choosing JURA BOT!\n\n` +
