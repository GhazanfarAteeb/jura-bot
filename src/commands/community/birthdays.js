  ...
  execute: async (message, args) => {
    ...
    try {
      ...
      for (const item of birthdays) {
        const bday = item.birthday; // The actual Birthday document
        const dateKey = `${bday.birthday.month}/${bday.birthday.day}`;
        if (!grouped[dateKey]) {
          grouped[dateKey] = [];
        }
        grouped[dateKey].push(bday);
      }
      ...
      for (const dateKey of sortedDates) {
        ...
        let value = '';
        for (const bday of birthdayList) {
          const member = await message.guild.members.fetch(bday.userId).catch(() => null);
          if (!member) continue;

          value += `${member.toString()}`;

          if (bday.showAge && bday.birthday.year) {
            const age = bday.getAge();
            if (age !== null) {
              value += ` • Turning ${age + 1}`;
            }
          }
