...
async function handleConfessionReplyModalSubmit(interaction, client) {
    ...
    // Determine if reply is anonymous
    const isAnonymous = confessionData.settings.anonymousReplies;
    const displayName = isAnonymous ? 'Anonymous' : interaction.user.displayName;
    const displayIcon = isAnonymous ? interaction.guild.iconURL() : interaction.user.displayAvatarURL();

    // Create reply embed
    const replyEmbed = new EmbedBuilder()
        .setAuthor({ 
            name: `Reply to Confession #${confessionNumber}`, 
            iconURL: displayIcon 
        })
        .setDescription(`"${replyContent}"`)
        .setColor('#7289da')
        .setFooter({ text: isAnonymous ? 'Anonymous Reply' : `Reply by ${displayName}` })
        .setTimestamp();

    // Try to reply to the original message
    try {
        const originalMessage = await channel.messages.fetch(confession.messageId);
        await originalMessage.reply({ embeds: [replyEmbed] });
    } catch (error) {
        // If original message not found, just send to channel
        await channel.send({ embeds: [replyEmbed] });
    }

    // Save the reply
    confession.replies = confession.replies || [];
    ...
    return interaction.reply({
        content: `✅ Your reply to confession #${confessionNumber} has been posted!`,
        ephemeral: true
    });
}
