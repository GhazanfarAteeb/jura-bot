...
export default {
  ...
  async execute(oldState, newState, client) {
    ...
    if (oldState.id === client.user.id && oldState.channel && newState.channel && oldState.channel.id !== newState.channel.id) {
      logger.info(`[VoiceStateUpdate] Bot was moved to a different channel in guild ${oldState.guild.id}`);
    ...
    if (botVoiceChannel.members.size === 1 && botVoiceChannel.members.has(client.user.id)) {
      logger.info(`[VoiceStateUpdate] Bot is alone in voice channel in guild ${oldState.guild.id}`);
      ...
      setTimeout(async () => {
        ...
        if (currentChannel && currentChannel.members.size === 1) {
          logger.info(`[VoiceStateUpdate] Disconnecting due to inactivity in guild ${oldState.guild.id}`);
